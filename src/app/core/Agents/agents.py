...
def _get_retrieval_agent():
    """Create the retrieval agent lazily (after env is configured)."""
    return create_agent(
...
def _get_summarization_agent():
    """Create the summarization agent lazily (after env is configured)."""
    return create_agent(
        model=create_chat_model(),
        tools=[],
...
def _get_verification_agent():
    """Create the verification agent lazily (after env is configured)."""
    return create_agent(
        model=create_chat_model(),
        tools=[],
...
def retrieval_node(state: QAState) -> QAState:
    ...
    result = _get_retrieval_agent().invoke(
...
def summarization_node(state: QAState) -> QAState:
    ...
    result = _get_summarization_agent().invoke(
...
Please verify and correct the draft answer, removing any unsupported claims."""
    ...
    result = _get_verification_agent().invoke(
        {"messages": [HumanMessage(content=user_content)]}
    )
    messages = result.get("messages", [])
